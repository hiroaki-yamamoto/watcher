...
serializer& storage::operator<<(serializer &out,const QVariant &data){
    ...
    if(data.canConvert(QMetaType::QString)) out<<(int)QMetaType::QString<<data.toString().toStdString();
    else if(data.canConvert(QMetaType::QVariantList)) out<<(int)QMetaType::QVariantList<<data.toList().toStdList();
        ...
        for(const QString &key:hash.uniqueKeys()) out<<key.toStdString()<<hash.values(key).toStdList();
    }else if(data.canConvert(QMetaType::QVariantMap)){
        QVariantMap mp=data.toMap();
        out<<(int)QMetaType::QVariantHash<<mp.uniqueKeys().size();
        for(const QString &key:mp.uniqueKeys()) out<<key.toStdString()<<mp.values(key).toStdList();
    }
    else throw invalid_argument("The data type is unserializable type.");
    return out;
}
    ...
    switch((QMetaType::Type)type){
        ...
        case QMetaType::QVariantHash:
        {
            ...
            for(int index=0;index<size;index++){
                ...
                in>>key>>values;
                for(const QVariant &value:values) hash.insertMulti(QString::fromStdString(key),value);
            }
            data.setValue(hash);
        }
            break;
        case QMetaType::QVariantMap:
        {
            ...
            for(int index=0;index<size;index++){
                ...
                list<QVariant> values;
                in>>key>>values;
                for(const QVariant &value:values) mp.insertMulti(QString::fromStdString(key),value);
            }
            data.setValue(mp);
        }
            break;
        default: throw logic_error("Undeserializable type."); break;
    }
    return in;
}
