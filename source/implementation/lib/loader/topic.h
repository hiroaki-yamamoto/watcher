...
namespace plugin {
    ...
    slots:
        ...
        // This function is for showing posting dialog, and post the response
        // when "Ok" button is clicked.
        virtual void post() = 0;
    signals:
        void post_finished(const response &res);
        void post_failed(const QNetworkReply::NetworkError err,
                         const QString &err_str);
        void get_responses_finished(
            const QVector<plugin::response *> &responses);
        void get_responses_failed(const QNetworkReply::NetworkError err,
                                  const QString &err_str);
        void get_response_warning(const QNetworkReply::NetworkError err,
                                  const QUrl &url, const QString &err_str);
    };
}
