...
#include "trace.h"
namespace yotsuba {
    board::board(std::mt19937 *mt, QNetworkAccessManager *accessManager,
                 QObject *parent)
        : plugin::board(parent) {
        if (mt == nullptr) {
        ...
        for (const QJsonValue &&page : root_array) {
            ...
            if (!el.contains("threads")) {
                emit this->get_topics_failed(QNetworkReply::UnknownContentError,
                                             generate_key_not_found("threads"));
                return;
            }
            ...
            if (!topic_list_value.isArray()) {
                emit this->get_topics_failed(
                    QNetworkReply::UnknownContentError,
                    generate_invalid_type("threads", "Array."));
                return;
            }
            QJsonArray &&topic_list = topic_list_value.toArray();
            for (QJsonArray::ConstIterator &&topic_ref =
                     topic_list.constBegin();
                 topic_ref < topic_list.constEnd(); topic_ref++) {
                if (!(*topic_ref).isObject()) {
                    emit this->get_topics_failed(
                        QNetworkReply::UnknownContentError,
                        generate_invalid_type("The element of \"threads\"",
                                              "Object"));
                    return;
                }
                QJsonObject &&topic_obj = (*topic_ref).toObject();
                // Check type of required attribute. (See attirbute.cxx for
                // details.)
                for (const QPair<QString, QJsonValue::Type> &
                    ...
                    if (topic_obj[required_attribute.first].type() !=
                ...
                yotsuba::topic *topic =
                    new yotsuba::topic(this->_accessmanager, this);
                topic->setTopicID(topic_obj["no"].toDouble());
                topic->setTopicURL(this->board_url().resolved(
                    "res/" + QString::number(topic->topicID())));
                topic->setIdentifier(QUuid::createUuidV5(
                    this->identifier(), QString::number(topic->topicID())));
                QString subject = topic_obj["sub"].toString(),
                        name = topic_obj["name"].toString();
                if (topic_obj.contains("trip") &&
                    topic_obj["trip"].type() == QJsonValue::String)
                    name += topic_obj["trip"].toString();
