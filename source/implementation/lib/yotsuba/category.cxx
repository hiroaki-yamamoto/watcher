...
#include "trace.h"
namespace yotsuba {
    category::category(std::mt19937 *mt, QNetworkAccessManager *manager,
                       QObject *parent)
    ...
    void category::getDataFinished(QNetworkReply *reply) {
        ...
        if (!this->_accessmanager->disconnect(
                 ...
                 SLOT(getDataFinished(QNetworkReply *)))) {
            qWarning()
                << "yotsuba: Yotsuba.Category:Signal disconnection failed.";
        }
        ...
        if (!boards_value.isArray()) {
            emit this->get_boards_failed(
                QNetworkReply::UnknownContentError,
                generate_invalid_type("boards", "Array"));
            return;
        }
        QJsonArray &&boards = boards_value.toArray();
        QVector<plugin::board *> board_list;
        for (QJsonArray::const_iterator &&board = boards.constBegin();
             board < boards.constEnd(); board++) {
            if (!(*board).isObject()) {
                emit this->get_boards_failed(
                    QNetworkReply::UnknownContentError,
                    generate_invalid_type("The element of \"boards\"",
                                          "Object"));
                return;
            }
            ...
            if (!board_object.contains("board")) {
                emit this->get_boards_failed(QNetworkReply::UnknownContentError,
                                             generate_key_not_found("board"));
                return;
            }
            if (!board_object.contains("title")) {
                emit this->get_boards_failed(QNetworkReply::UnknownContentError,
                                             generate_key_not_found("title"));
                return;
            }
            if (!board_object.contains("ws_board")) {
                emit this->get_boards_failed(
                    QNetworkReply::UnknownContentError,
                    generate_key_not_found("ws_board"));
                return;
            }
            QJsonValue &&board_dir = board_object["board"],
            ...
            if (!board_dir.isString()) {
                emit this->get_boards_failed(
                    QNetworkReply::UnknownContentError,
                    generate_invalid_type("board", "Object"));
                return;
            }
            if (!board_title.isString()) {
                emit this->get_boards_failed(
                    QNetworkReply::UnknownContentError,
                    generate_invalid_type("title", "String"));
                return;
            }
            if (!ws_board.isDouble()) {
                emit this->get_boards_failed(
                    QNetworkReply::UnknownContentError,
                    generate_invalid_type("ws_board", "Double"));
                return;
            }
            if ((int)this->_ws == (int)ws_board.toDouble()) {
                unsigned int random_number = (*this->_mt)();
                yotsuba::board *yotsuba_board =
                    new yotsuba::board(this->_mt, this->_accessmanager, this);
                yotsuba_board->setBoardURL(QUrl(QString(
                    "http://boards.4chan.org/%1/").arg(board_dir.toString())));
                yotsuba_board->setTitle(board_title.toString());
                yotsuba_board->setBoardDirName(board_dir.toString());
                yotsuba_board->setIdentifier(QUuid::createUuidV5(
                    this->identifier(),
                    QByteArray((char *)&random_number, sizeof(random_number))));
                yotsuba_board->setDescription(
                    yotsuba_board->board_url().toString());
