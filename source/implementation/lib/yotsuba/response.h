...
namespace yotsuba {
     ...
     * First, to avoid using include compiler instruction, all variables and
     * return values are references or pointers.
     */
    class response : public plugin::response {
       ...
       public:
        response(QNetworkAccessManager *accessManager,
    ...
    signals:
        void fetchingImageFailed(QNetworkReply::NetworkError err,
                                 const QUrl &url, const QString &errStr);
       private
    slots:
