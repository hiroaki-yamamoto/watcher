...
namespace yotsuba {
    ...
    QUrl topic_list_url(const QString &board_dir) {
        QUrl &&result =
            api_domain().resolved(QUrl("/" + board_dir + "/catalog.json"));
        qDebug() << "yotsuba: 4chan API topic list URL:" << result;
        return result;
    }
    QUrl response_list_url(const QString &board_dir,
                           const qulonglong &topicID) {
        QUrl &&result = api_domain().resolved(QUrl(
            "/" + board_dir + "/res/" + QString::number(topicID) + ".json"));
        qDebug() << "yotsuba: 4chan API response list URL:" << result;
        return result;
    }
    QUrl image_url(const QString &board_dir, const qulonglong &tim,
                   const QString &ext) {
        // Adjusted 4chan api
        QUrl &&result = QUrl(QString("http://t.4cdn.org/%1/thumb/%2s%3")
                                 .arg(board_dir, QString::number(tim), ".jpg"));
