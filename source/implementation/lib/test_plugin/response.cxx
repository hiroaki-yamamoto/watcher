#include "response.h"
namespace test {
    response::response(const QString &title, const QString &body,
                       const QString &author, const QString &email,
                       const QDateTime &creation_date, const QUrl &res_url,
    ...
    void response::fetchImage(const QUrl &link_url, const QImage &image) {
        QUuid &&uuid =
            QUuid::createUuidV5(this->identifier(), link_url.toString());
