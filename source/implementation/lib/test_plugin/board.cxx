...
namespace test {
    board::board(const QString &name, const QUuid &id, QObject *parent)
        : plugin::board(parent) {
        ...
        this->_board_url =
            QString("http://example.com/boards/%1").arg(copied_name);
    ...
    void board::get_topics() {
        ...
        for (quint16 count = 0; count < 100; count++) {
            ...
            topics << new test::topic(
                          QString("Test Topic Title: %1").arg(count),
                          "Anonymous", topic_id,
                          this->board_url().resolved(
                              QString("topic-%1").arg(topic_id.toString())),
                          this);
        }
