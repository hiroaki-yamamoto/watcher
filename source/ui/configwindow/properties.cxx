...
namespace ui {
    ...
    void ConfigDialog::_setVariantToUI(QWidget *ui, const QVariant &value) {
        ...
        if (class_name == "QCheckBox" || class_name == "QRadioButton")
            QMetaObject::invokeMethod(ui, "setChecked",
                                      Q_ARG(bool, value.toBool()));
        else if (class_name == "QSpinBox")
            QMetaObject::invokeMethod(ui, "setValue",
                                      Q_ARG(int, value.toInt()));
        else if (class_name == "ui::filechooser")
            QMetaObject::invokeMethod(ui, "setPath",
                                      Q_ARG(QString, value.toString()));
        else if (class_name == "ui::theme_scroll_area") {
            switch (value.type()) {
                case QMetaType::QString:
                    QMetaObject::invokeMethod(
                        ui, "selectDir", Q_ARG(QDir, QDir(value.toString())));
                    break;
                case QMetaType::Int: {
                    int index = value.toInt();
                    ThemeScrollArea *theme_area =
                        qobject_cast<decltype(theme_area)>(ui);
                    if (theme_area->children_size() <= index) {
                        qWarning() << this << "Out of range.";
                        break;
                    }
                    ThemePanel *panel = qobject_cast<decltype(panel)>(
                        (*theme_area)[value.toInt()]);
                    panel->setSelected(true);
                } break;
                default:
                    qWarning() << this
                               << "The type of specified value is invalid.";
                    break;
            }
        }
    }
