...
namespace ui {
    ...
    FileChooser::FileChooser(const QFileInfo &file_path, const ChooseMode mode,
                             const QString &objName, QWidget *parent)
        : QWidget(parent) {
        this->setObjectName(objName);
        if (!file_path.exists())
            qWarning() << this << file_path.filePath() << " doesn't exist.";
        else
            qWarning() << this << "FilePath" << file_path.absoluteFilePath();
        ...
        connect(this->_refbutton, SIGNAL(clicked()),
                SLOT(_refbutton_clicked()));
    }
    ...
    void FileChooser::setPath(const QString &path) {
        ...
        bool cancel = false;
        if (!now_path.exists()) {
            qWarning() << this << now_path.absoluteFilePath()
                       << " doesn't exist.";
        }
        if (this->_mode == FileChooser::Dir && !now_path.isDir())
            qWarning() << this << now_path.absoluteFilePath()
                       << " is not a directory.";
        else if (this->_mode == FileChooser::File && now_path.isDir())
            qWarning() << this << now_path.absoluteFilePath()
                       << "is not a file.";
        ...
        if (previous_path != now_path) {
            this->_info = now_path;
            qDebug() << this
                     << "Previous Path:" << previous_path.absoluteFilePath();
    ...
    void FileChooser::_refbutton_clicked() {
        ...
        switch (this->_mode) {
            case FileChooser::File:
                str = QFileDialog::getOpenFileName(this, "Open", QString(),
                                                   this->_filter);
                break;
            case FileChooser::Dir:
                str = QFileDialog::getExistingDirectory(this, "Open",
                                                        this->_filter);
                break;
        }
