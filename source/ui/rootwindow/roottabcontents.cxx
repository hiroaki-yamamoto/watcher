...
namespace ui {
    RootTabContents::RootTabContents(plugin::root *plugin_root,
                                     ui::RootWindow *parent)
        : TabContentsBase(plugin_root->title(), plugin_root->identifier(),
                          parent) {
        this->_root = plugin_root;
        this->_state = RootTabContents::Category;

        connect(this->_root,
                SIGNAL(get_categories_finished(QVector<plugin::category *>)),
    ...
    bool RootTabContents::categories_empty() const {
        return this->_category_hash.isEmpty();
    }
    bool RootTabContents::boards_empty() const {
        return this->_board_hash.isEmpty();
    }
    void RootTabContents::_get_category_completed(
        const QVector<plugin::category *> &categories) {
        this->_category_hash.clear();
        for (plugin::category *category : categories) {
            connect(category,
                    SIGNAL(get_boards_finished(QVector<plugin::board *>)),
                    SLOT(_get_boards_completed(QVector<plugin::board *>)));
            connect(
                category,
                SIGNAL(get_boards_failed(QNetworkReply::NetworkError, QString)),
                SLOT(_get_boards_failed(QNetworkReply::NetworkError, QString)));
            this->_category_hash.insert(
                qMakePair(category->identifier(), category->title()), category);
        }
        if (this->_state == ui::RootTabContents::Category)
            this->_set_category();
    }
    void RootTabContents::_get_boards_completed(
        const QVector<plugin::board *> &boards) {
        this->_board_hash.clear();
        for (plugin::board *board : boards) {
            this->_board_hash.insert(
                qMakePair(board->identifier(), board->title()), board);
        }
    ...
    QString RootTabContents::TabName() const {
        if (this->_tabcontents->property("title").type() !=
            QMetaType::QString) {
    ...
    void RootTabContents::_button_clicked(const QVariant &button_var) {
        ...
        this->_contentsName.insert(
            this->_state,
            qMakePair(
                QUuid(QByteArray(button_var.value<QQuickItem *>()
                                     ->property("uuid")
                                     .toString()
                                     .toUtf8())),
    ...
    void RootTabContents::_do_switch(const SwitchInstruction sw) {
        ...
        if (this->_hasAnimation &&
            (sw != RootTabContents::ContentsSwitch ||
             this->_state == RootTabContents::Category)) {
            connect(this->_tabcontents, SIGNAL(hideAnimationCompleted()),
                    SLOT(_process_switch_instruction()));
            if (!QMetaObject::invokeMethod(this->_tabcontents, "hide"))
                this->_process_switch_instruction();
        } else
            this->_process_switch_instruction();
    }
    void RootTabContents::_process_switch_instruction() {
        switch (this->_sw_inst) {
            case RootTabContents::ContentsSwitch:
                this->_switch_contents();
                break;
            case RootTabContents::Back:
                this->_back();
                break;
            case RootTabContents::Forward:
                this->_forward();
                break;
            case RootTabContents::Reload:
                this->_reload();
                break;
        }
        if (this->_hasAnimation)
    ...
    void RootTabContents::_switch_contents() {
        switch (this->_state) {
            case RootTabContents::Category:
                if (this->_category_hash.contains(
                        this->_contentsName[RootTabContents::Category])) {
                    ...
                    qWarning() << this << "Value of " << this->_contentsName
                               << " couldn't be found.";
                    qWarning() << "    State:" << this->_state;
                }
                break;
            case RootTabContents::Board:
                if (this->_board_hash.contains(
                        this->_contentsName[RootTabContents::Board])) {
                    emit this->topicMode(
                        this->_board_hash
                            [this->_contentsName[RootTabContents::Board]]);
                } else {
    ...
    void RootTabContents::_addButton(const QString &button_text,
                                     const QString &detailed_text,
                                     const QUuid &id) {
        QMetaObject::invokeMethod(this->_tabcontents, "addButton",
                                  Q_ARG(QVariant, button_text),
                                  Q_ARG(QVariant, detailed_text),
                                  Q_ARG(QVariant, QVariant(id.toString())));
    }
    void RootTabContents::_clearButtons() {
        QMetaObject::invokeMethod(this->_tabcontents, "clearButtons");
    }

    void RootTabContents::_set_category() {
        this->_clearButtons();
        std::list<plugin::category *> &&categories =
            this->_category_hash.values().toStdList();
        categories.sort([](
            const plugin::category * a,
            const plugin::category *
                b)->bool { return a->title().compare(b->title()) < 0; });
        for (const plugin::category *category : categories) {
            this->_addButton(category->title(), category->description(),
                             category->identifier());
        }
        if (this->_hasAnimation)
            QMetaObject::invokeMethod(this->_tabcontents, "show");
    }
    void RootTabContents::_set_board() {
        this->_clearButtons();
        std::list<plugin::board *> &&boards =
            this->_board_hash.values().toStdList();
        boards.sort([](const plugin::board * a, const plugin::board * b)
                        ->bool { return a->title().compare(b->title()) < 0; });
        for (const plugin::board *board : boards) {
            this->_addButton(board->title(), board->description(),
                             board->identifier());
        }
        if (this->_hasAnimation)
            QMetaObject::invokeMethod(this->_tabcontents, "show");
    ...
    // Error case
    void RootTabContents::_fetch_failed(const QString &type,
                                        const QString &err_str) {
        QMessageBox::warning(
            nullptr, tr("%1:Fetch Error").arg(this->_root->title()),
            tr("Plugin Name: %1<br />"
               "Couldn't fetch list of %2.<br />"
               "Reason: %3").arg(this->_root->title(), type, err_str));
    }

    void RootTabContents::_get_category_failed(
        const QNetworkReply::NetworkError err, const QString &err_str) {
        this->_fetch_failed(tr("categories"), err_str);
    }
    void RootTabContents::_get_boards_failed(
        const QNetworkReply::NetworkError err, const QString &err_str) {
