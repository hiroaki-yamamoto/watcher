...
namespace ui {
    TabWindowBase::TabWindowBase(const QString &title, const QIcon &icon,
    ...
    TabContentsBase *TabWindowBase::_getCurrentTabContents() {
        ...
        if (!this->_tabcontents.contains(tab_key)) {
            qDebug() << this << "TabContent Named:" << tab_title
                     << " couldn't be found.";
            return nullptr;
        } else
    ...
    QQuickItem *TabWindowBase::addTab(const QString &title, const QUuid &uuid) {
        ...
        if (!QMetaObject::invokeMethod(
                 this->rootObject(), "addTab", Q_RETURN_ARG(QVariant, variant),
    ...
    void TabWindowBase::deleteLater() {
        if (this->_tabcontents.size() > 0) {
            for (const QPair<QString, QUuid> &key :
                 this->_tabcontents.uniqueKeys()) {
                for (TabContentsBase *value : this->_tabcontents.values(key))
                    value->deleteLater();
            }
