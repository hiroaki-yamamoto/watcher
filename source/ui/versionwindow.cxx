...
namespace ui {
    VersionWindow::VersionWindow(const QString &title, const QIcon &icon,
        ...
        this->rootContext()->setContextProperty("applicationName",
        ...
        for (const QString &objName : this->_children.uniqueKeys()) {
            if (objName == "body")
                connect(this->_children.value(objName),
                        SIGNAL(linkActivated(QString)),
                        SLOT(_linkActivated(QString)));
            else if (objName == "close")
                connect(this->_children.value(objName), SIGNAL(clicked()),
                        SLOT(_closeButtonClicked()));
        }
    ...
    void VersionWindow::_propertyChanged(const QString &name,
                                         const QVariant &prev,
                                         const QVariant &now) {
        Q_UNUSED(prev);
#ifdef EXPERIMENT
        if (name == default_value::setting_default::name_theme_selected_dir()) {
            if (now.type() == QMetaType::QString)
                this->_loadQMLFile(
                    QFileInfo(now.toString(), "VersionWindow.qml"));
            else
                qWarning()
                    << this << ": Setting key:"
    ...
    void VersionWindow::_linkActivated(const QString &link) {
