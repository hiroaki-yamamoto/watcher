...
namespace ui {
    ...
    class RootTabContents : public TabContentsBase {
       ...
       public:
        RootTabContents(plugin::root *plugin_root,
                        ui::RootWindow *parent = nullptr);
        enum ViewState {
            Category,
            Board
        };
    ...
    slots:
        ...
        void _get_category_failed(const QNetworkReply::NetworkError err,
                                  const QString &err_str);
        void _get_boards_failed(const QNetworkReply::NetworkError err,
                                const QString &err_str);
        ...
        void _forward();
        void _reload();
        void _addButton(const QString &button_text,
                        const QString &detailed_text = QString(),
                        const QUuid &id = QUuid());
        void _clearButtons();

       private:
        enum SwitchInstruction {
            Back,
            Forward,
            Reload,
            ContentsSwitch
        ...
        plugin::root *_root;
        QHash<ui::RootTabContents::ViewState, QPair<QUuid, QString>>
            _contentsName;
        ViewState _state;
    };
}
