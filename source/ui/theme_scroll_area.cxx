...
namespace ui {
    ThemeScrollArea::ThemeScrollArea(const QVector<QDir> &theme_root_dirs,
                                     QWidget *parent)
        : MultipleScrollArea(parent) {
        this->_theme_root_dirs = new QVector<QDir>();
        this->setThemeRootDirs(theme_root_dirs);
    }
    ThemeScrollArea::ThemeScrollArea(QWidget *parent)
        : MultipleScrollArea(parent) {
        this->_theme_root_dirs = new QVector<QDir>();
    ...
    void ThemeScrollArea::setThemeRootDirs(
        const QVector<QDir> &theme_root_dirs) {
    ...
    QVector<QDir> ThemeScrollArea::themeRootDirs() const {
        return (*this->_theme_root_dirs);
    }
    void ThemeScrollArea::checkEntry() {
        this->clear();
        for (QDir &dir : *this->_theme_root_dirs) {
            if (!dir.exists()) {
                qWarning() << this << "Theme root dir:" << dir.path()
                           << "doesn't exist.";
                continue;
            }
            ...
            for (QFileInfo &info : dir.entryInfoList(
                     nameFilter, QDir::NoDotAndDotDot | QDir::Dirs)) {
                qDebug() << this << "Theme dir found:" << info.filePath();
                ui::ThemePanel *panel =
                    new ui::ThemePanel(QDir(info.filePath()), this);
                connect(panel, SIGNAL(selectionChanged(bool)),
    ...
    void ThemeScrollArea::selectionChanged(bool selected) {
        ...
        if (this->_previous_selected != nullptr &&
            this->_previous_selected != sender && selected) {
            this->_previous_selected->disconnect(
                SIGNAL(selectionChanged(bool)));
            this->_previous_selected->setSelected(false);
            connect(this->_previous_selected, SIGNAL(selectionChanged(bool)),
                    SLOT(selectionChanged(bool)));
