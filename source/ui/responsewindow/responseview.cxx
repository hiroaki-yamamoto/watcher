...
namespace ui {
    ResponseView::ResponseView(plugin::topic *topic, TabContentsBase *parent)
        ...
        this->_topic = topic;
        connect(this->_topic,
                SIGNAL(get_responses_finished(QVector<plugin::response *>)),
                SLOT(responseLoaded(QVector<plugin::response *>)));
        connect(
            this->_topic,
            SIGNAL(get_responses_failed(QNetworkReply::NetworkError, QString)),
        ...
        if (this->_tabcontents->property("hasAnimation").toBool()) {
            connect(this->_tabcontents, SIGNAL(hideAnimationCompleted()),
                    this->_topic, SLOT(get_responses()));
        ...
        this->_tabcontents->setProperty(
            "topicURL", QVariant(topic->topic_url().toString()));
    }
    ...
    void ResponseView::responseLoaded(
        const QVector<plugin::response *> &responses) {
        plugin::topic *topic = qobject_cast<decltype(topic)>(this->sender());
        if (this->_tabcontents->property("hasAnimation").toBool()) {
            this->_tabcontents->disconnect(SIGNAL(hideAnimationCompleted()),
                                           topic, SLOT(get_responses()));
        }
    ...
    void ResponseView::responseFetchingFailed(
        const QNetworkReply::NetworkError err, const QString &err_str) {
        Q_UNUSED(err)
        QMessageBox::warning(nullptr, tr("Response Fetching failed"),
                             tr("<p>Fetching responses failed. Please make "
                                "sure the response page exists exactly.</p>"
                                "<p>Error message:%1</p>").arg(err_str));
    }
    ...
    void ResponseView::addItem(plugin::response *res) {
        ...
        if ((currentPanel = this->_childrenItems
                                [qMakePair(res->title(), res->identifier())]) !=
            nullptr) {
            currentPanel->deleteLater();
        }
        this->_childrenItems[qMakePair(res->title(), res->identifier())] =
            new ResponsePanel(res, this);
    }
    void ResponseView::updateImageInfo() {
        plugin::response *res =
            qobject_cast<plugin::response *>(this->sender());
        if (res != nullptr && res->images() != nullptr) {
            ...
            for (QUuid &imageUUID : images->uniqueKeys()) {
                ...
                imageInfo["SourceURI"] = QString("image://%1/%2").arg(
                    uuid.toString().replace("{", "").replace("}", ""),
    ...
    QQuickItem *ResponseView::_addItem(
        const QString &title, const QString &author, const QString &email,
        const QDateTime &post_time, const QString &body, const QUuid &uuid,
        const QUrl &responseURL, manager::ImageManager *images) {
        ResponseTabContents *parentTabContents =
            qobject_cast<ResponseTabContents *>(this->_parentTab);
        ...
        */
        QString img_providerID =
            uuid.toString().replace("{", "").replace("}", "");
        if (windowEngine->imageProvider(img_providerID) == nullptr) {
            windowEngine->addImageProvider(
                uuid.toString().replace("{", "").replace("}", ""), images);
        }
        ...
        bool succeeded = QMetaObject::invokeMethod(
            this->_tabcontents, "addResponse",
            Q_RETURN_ARG(QVariant, invoke_result),
            Q_ARG(QVariant, QVariant(title)), Q_ARG(QVariant, QVariant(author)),
            Q_ARG(QVariant, QVariant(email)),
            Q_ARG(QVariant, QVariant(post_time.toString())),
            Q_ARG(QVariant, QVariant(body)), Q_ARG(QVariant, QVariant(uuid)),
            Q_ARG(QVariant, responseURL), Q_ARG(QVariant, imageInfoList));
