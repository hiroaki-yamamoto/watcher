...
namespace ui{
    ...
    void ResponseWindow::addTopic(plugin::topic *th){
        ...
        if(boa==nullptr){
            QMessageBox::critical(nullptr,tr("Internal Error"),tr("The address of board is null"));
            qWarning()<<this<<"Topic info:{ID:"<<th->identifier()<<", title:"<<th->title()<<"}";
            return;
        }
        plugin::category *cat=qobject_cast<decltype(cat)>(boa->parent());
        if(cat==nullptr){
            QMessageBox::critical(nullptr,tr("Internal Error"),tr("The address of category is null"));
            qWarning()<<this<<"Board info:{ID:"<<boa->identifier()<<", title:"<<boa->title()<<"}";
            return;
        }
        plugin::root *rot=qobject_cast<decltype(rot)>(cat->parent());
        if(rot==nullptr){
            QMessageBox::critical(nullptr,tr("Internal Error"),tr("The address of root is null"));
            qWarning()<<this<<"Category Info:{ID:"<<cat->identifier()<<", title:"<<boa->title()<<"}";
            return;
        }
        QPair<QString,QUuid> &&key=qMakePair(rot->title(),rot->identifier());
        if(this->_tabcontents.contains(key)){
            ResponseTabContents *content=qobject_cast<decltype(content)>(this->_tabcontents[key]);
            if(content!=nullptr){
                content->addTopic(th);
            }else{
                content=new ResponseTabContents(rot->title(),rot->identifier(),this);
                this->_tabcontents[key]=content;
                content->addTopic(th);
            }
        }else{
            ResponseTabContents *content=new ResponseTabContents(rot->title(),rot->identifier(),this);
            this->_tabcontents[key]=content;
            content->addTopic(th);
        }
    }
    ...
    void ResponseWindow::_reload(){
        ResponseTabContents *tab=qobject_cast<decltype(tab)>(this->_getCurrentTabContents());
        if(tab==nullptr){
